...
/*eslint-disable */
// copy n pasted this from stack overflow and it works. aint reversing the obfuscation cause that feels unnecessary
...
exports.loadImage = async (url, w, h, getColor = false) => {
  const d = await axios({ url, method: 'get', responseType: 'arraybuffer' })
  const img = new Image()
  img.src = d.data
  img.weight = w
  img.height = h
  return getColor ? [img, d.headers['x-cover-color']] : img
}
